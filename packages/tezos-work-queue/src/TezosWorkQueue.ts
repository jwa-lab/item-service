...
export class TezosWorkQueue extends JetStreamConsumer {
    ...
    async handle<WalletParamsWithKind>({ msg, data }: JetStreamMessage<WalletParamsWithKind>): Promise<void> {
        if (!data) {
            msg.term();

            this.logger.error(`Tezos Work Queue is expecting an operation but didn't receive anything.`);
            return;
        } else {
            msg.working();
            ...
            this.logger.debug(
                `Successfully processed ${JSON.stringify(data)}`
            );
